...
test('blandet state lagrer business atomisk og admininnstillinger separat', async () => {
  const { localStorage, requests, window } = setup();
  localStorage.setItem('homar_server_state_revision', '0');
  localStorage.setItem('homar_budsjet', JSON.stringify({ bank:0, wafi:500 }));
  localStorage.setItem('homar_lager', JSON.stringify([{ type:'B', qty:3 }]));

  await window.fetch('https://homar-logistik.onrender.com/api/state', {
    method:'PUT',
    headers:{ 'Content-Type':'application/json' },
    body:JSON.stringify({
      state:{ homar_lager:[{ type:'B', qty:3 }], homar_custom_labels:{ '#tab':'LAGER' } }
    })
  });

  const atomic = requests.find(request => request.url.endsWith('/api/state/business'));
  const settings = requests.find(request => request.url.endsWith('/api/state'));
  assert.ok(atomic);
  assert.ok(settings);
  assert.deepEqual(JSON.parse(settings.options.body), {
    state:{ homar_custom_labels:{ '#tab':'LAGER' } }
  });
});
